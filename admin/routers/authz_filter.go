...
func getModuleOrganizationObject(path string, method string, queryOrganization string, body []byte) (string, string, bool) {
	...
	if path == "/api/organization-master-data-quality/directory" || path == "/api/organization-master-data-quality/remediation-plan" || path == "/api/organization-master-data-quality/remediation-action-drafts" || path == "/api/organization-master-data-quality/remediation-preflight" || path == "/api/organization-master-data-quality/remediation-approval-preview" {
		return strings.TrimSpace(queryOrganization), "", true
	}
