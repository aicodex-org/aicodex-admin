...
function defaultOwnerHandoffs(input = {}, status) {
    const adminHandoffs = [
    ...
    return [defaultHandoff(status === "missing-api-diagnostics" ? "api_diagnostics_evidence_missing" : "controlled_smoke_evidence_not_checked", status === "missing-api-diagnostics" ? "api_diagnostics_owner" : "admin_operator")];
}
...
function redactedEvidenceHints(input = {}) {
    const hints = [
...
function buildResult(status, input, options = {}, extra = {}) {
...
function isApiDiagnosticsReady(evidence = {}) {
...
function adminMissingPrerequisites(input = {}) {
    const missing = [];
...
function apiMissingPrerequisites(input = {}) {
...
function createGatewayProjectionControlledSmokeEvidenceReadiness(input = {}, options = {}) {
