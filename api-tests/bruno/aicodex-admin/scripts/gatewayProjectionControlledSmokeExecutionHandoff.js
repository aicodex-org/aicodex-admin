...
function ownerLimitFromHandoff(handoff = {}, fallbackAlias = "controlled_smoke_execution_prerequisite_missing", fallbackOwner = "admin_operator") {
  return {
...
function ownerLimitsFromSummaries(input = {}) {
  const summaries = [
...
function evidencePackageMetadata(input = {}, options = {}) {
  return {
...
function isEvidenceReady(summary = {}) {
...
function isRunbookReady(summary = {}) {
  return summary.status === "ready" &&
    summary.release === "release_after_report" &&
...
function isOperatorRemediationReady(summary = {}) {
  return summary.status === "ready-for-operator-handoff" ||
    summary.status === "not-required" ||
    summary.status === "ready";
}

function isRemediationResultReady(summary = {}) {
  return summary.status === "ready-for-controlled-smoke-evidence-review";
}
...
function firstBlockingHandoff(input = {}) {
  const summaries = [
    input.preflightSummary,
    input.evidenceReadinessSummary,
    input.releaseRunbookSummary,
...
function allEvidenceAliases(input = {}) {
  function evidenceAliasesFrom(value = {}) {
    if (!hasObject(value)) {
      return [];
    }
    return unique([
...
function createGatewayProjectionControlledSmokeExecutionHandoff(input = {}, options = {}) {
