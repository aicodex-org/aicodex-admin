...
function ownerLimitsFromDecision(summary = {}, fallbackStatus = "blocked") {
    ...
    if (limits.length > 0) {
        return limits.map((item) => ownerLimit(actionableAlias(item.alias, actionableAlias(summary.blockerAlias, "controlled_smoke_operator_action_blocked")), item.nextAction || nextActionFor(fallbackStatus), item.minimumUnblockCondition || item.condition || "稳定 owner-scoped alias 已清除且无 red-line signal", item.owner || "admin_operator"));
    }
    return [ownerLimit(actionableAlias(summary.blockerAlias, "controlled_smoke_operator_action_blocked"), nextActionFor(fallbackStatus), "稳定 blocker/remediation alias 已清除，且 decision package 仍为本地脱敏摘要")];
}
...
function baseResult(actionStatus, input = {}, options = {}, overrides = {}) {
    ...
    const ownerHandoffLimits = overrides.ownerHandoffLimits || [ownerLimit(overrides.blockerAlias || "controlled_smoke_operator_action_handoff", nextActionFor(actionStatus), actionStatus === READY_ACTION_STATUS
...
function createGatewayProjectionControlledSmokeOperatorActionHandoff(input = {}, options = {}) {
    ...
    if (hasSensitiveEvidence(input)) {
        return baseResult("blocked", {}, options, {
            ...
            ownerHandoffLimits: [
                ownerLimit("sanitization_failed", "替换疑似敏感 operator action evidence", "action package 只保留脱敏 alias、状态、计数、owner 和最小解除条件"),
            ],
        });
    }
    const decision = input.operatorDecisionHandoffSummary;
    ...
    if (unknown) {
        return baseResult("blocked", input, options, {
            ...
            ownerHandoffLimits: [
                ownerLimit("unknown_controlled_smoke_operator_action_alias", "替换未知 operator action alias", "unknown alias 已替换为 spec/test 中定义的稳定 Admin owner handoff alias"),
            ],
        });
    }
