...
function ownerLimitsFromSummary(summary = {}, fallbackStatus = "blocked") {
    ...
    if (limits.length > 0) {
        return limits.map((item) => ownerLimit(item.alias || summary.blockerAlias || "controlled_smoke_operator_decision_blocked", item.nextAction || nextStepsFor(fallbackStatus)[0], item.minimumUnblockCondition || item.condition || "稳定 owner-scoped alias 已清除且无 red-line signal", item.owner || "admin_operator"));
    }
    return [ownerLimit(summary.blockerAlias || "controlled_smoke_operator_decision_blocked", nextStepsFor(fallbackStatus)[0], "稳定 blocker/remediation alias 已清除，且输入仍为本地脱敏摘要")];
}
...
function decisionPackageMetadata(input = {}, options = {}) {
    return {
...
function baseResult(status, input = {}, options = {}, overrides = {}) {
    const ownerHandoffLimits = overrides.ownerHandoffLimits || [ownerLimit(overrides.blockerAlias || "controlled_smoke_operator_decision_handoff", status === READY_DECISION_STATUS ? "交接本地脱敏 operator decision package" : "补齐或修正本地脱敏 operator decision package 输入", status === READY_DECISION_STATUS
...
function isReady(summary = {}, expectedStatus) {
 ...
 */
function createGatewayProjectionControlledSmokeOperatorDecisionHandoff(input = {}, options = {}) {
    ...
    if (hasSensitiveEvidence(input)) {
        return baseResult("blocked", {}, options, {
            ...
            ownerHandoffLimits: [
                ownerLimit("sanitization_failed", "替换疑似敏感 operator decision evidence", "decision package 只保留脱敏 alias、状态、计数、owner 和最小解除条件"),
            ],
        });
    }
    ...
    if (missing.length > 0) {
        const blocking = summaries(input).find((summary) => ((summary.blockerAlias && summary.blockerAlias !== "none") ||
    ...
    if (unknown) {
        return baseResult("blocked", input, options, {
            ...
            ownerHandoffLimits: [
                ownerLimit("unknown_controlled_smoke_operator_decision_alias", "替换未知 operator decision alias", "unknown alias 已替换为 spec/test 中定义的稳定 Admin owner handoff alias"),
            ],
        });
    }
