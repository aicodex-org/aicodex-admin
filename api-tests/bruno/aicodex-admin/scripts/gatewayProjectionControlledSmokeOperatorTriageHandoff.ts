...
function triagePackageMetadata(options: LooseRecord = {}) {
  return {
...
function defaultOwnerLimits(status, alias = "controlled_smoke_operator_triage_handoff") {
  if (status === READY_TRIAGE_STATUS) {
    return [ownerLimit(
      ...
      "release summary handoff 与 result evidence handoff 均为 ready，且无 red-line signal"
    )];
  }
  return [ownerLimit(
    alias,
    "清除 stable blocker 后重跑 operator triage handoff",
...
function statusFromReleaseSummary(summary: LooseRecord = {}) {
...
function ownerLimitsFromSummary(summary: LooseRecord = {}, fallbackStatus = "blocked") {
  ...
  if (limits.length > 0) {
    return limits.map((item) => ownerLimit(
      item.alias || summary.blockerAlias || "controlled_smoke_operator_triage_blocked",
      item.nextAction || nextStepsFor(fallbackStatus)[0],
      item.minimumUnblockCondition || "稳定 owner-scoped alias 已清除且无 red-line signal",
 ...
 */
function createGatewayProjectionControlledSmokeOperatorTriageHandoff(input: LooseRecord = {}, options: LooseRecord = {}) {
  ...
  if (hasSensitiveEvidence(input)) {
    return baseResult("blocked", {}, options, {
      ...
      ownerHandoffLimits: [
        ownerLimit(
          "sanitization_failed",
          "替换疑似敏感 operator triage evidence",
  ...
  if (unknown) {
    return baseResult("blocked", input, options, {
      ...
      ownerHandoffLimits: [
        ownerLimit(
          "unknown_controlled_smoke_operator_triage_alias",
          "替换未知 operator triage alias",
