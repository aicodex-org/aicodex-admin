...
function defaultHandoff(decision, alias) {
    switch (decision) {
...
function buildResult(decision, reason, aliases = [], handoffs = [], options = {}) {
...
function isAdminReleaseReady(releaseDecision = {}) {
...
function isApiDiagnosticsReady(apiDiagnostics = {}) {
...
function createGatewayProjectionControlledSmokePreflightHandoff(input = {}, options = {}) {
    if (containsSensitiveEvidence(input)) {
        return buildResult("blocked-by-contract-or-redaction", "sensitive_or_raw_evidence_present", ["sanitization_failed"], [defaultHandoff("blocked-by-contract-or-redaction", "sanitization_failed")], options);
    }
    ...
    if (!adminReleaseDecisionHandoff || !adminReadinessSummary || !apiDiagnosticsDecision) {
        return buildResult("not-checked", "controlled_smoke_preflight_required_evidence_not_checked", ["controlled_smoke_preflight_evidence_not_checked"], [defaultHandoff("not-checked")], options);
    }
    ...
    if (sourceAlias) {
        return buildResult("blocked-by-admin-source-freshness", "admin_source_freshness_blocked", [sourceAlias], adminReadinessSummary.handoffs, options);
    }
    const mappingAlias = firstAliasMatching(readinessAliases, MAPPING_READINESS_ALIASES);
    if (mappingAlias) {
        return buildResult("blocked-by-mapping-readiness", "admin_mapping_readiness_blocked", [mappingAlias], adminReadinessSummary.handoffs, options);
    }
    if (adminReadinessSummary.status === "blocked" || readinessAliases.length > 0) {
        return buildResult("blocked-by-contract-or-redaction", "admin_readiness_unknown_alias_blocked", readinessAliases.length > 0 ? readinessAliases : ["admin_readiness_blocked"], adminReadinessSummary.handoffs, options);
    }
    if (!isApiDiagnosticsReady(apiDiagnosticsDecision)) {
        const aliases = aliasesFrom(apiDiagnosticsDecision);
        return buildResult("blocked-by-api-diagnostics", "api_diagnostics_decision_blocked", aliases.length > 0 ? aliases : ["api_diagnostics_blocked"], apiDiagnosticsDecision.ownerHandoffs, options);
    }
    if (!isAdminReleaseReady(adminReleaseDecisionHandoff)) {
        const aliases = aliasesFrom(adminReleaseDecisionHandoff);
        return buildResult("blocked-by-admin-release-decision", "admin_release_decision_blocked", aliases.length > 0 ? aliases : ["admin_release_decision_blocked"], adminReleaseDecisionHandoff.ownerHandoffs, options);
    }
    return buildResult("ready-for-controlled-smoke-prep", "sanitized_owner_evidence_ready_for_controlled_smoke_prep", [], [defaultHandoff("ready-for-controlled-smoke-prep")], options);
}
module.exports = {
