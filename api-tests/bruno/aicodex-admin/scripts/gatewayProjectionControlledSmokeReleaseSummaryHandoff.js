...
function ownerLimitsFromResultEvidence(summary = {}) {
    const handoffs = Array.isArray(summary.ownerHandoffLimits) ? summary.ownerHandoffLimits : [];
    if (handoffs.length > 0) {
        return handoffs.map((item) => ownerLimit(item.alias || summary.blockerAlias || "controlled_smoke_result_evidence_handoff", item.nextAction || "补齐 result evidence handoff 后重跑 release summary handoff", item.minimumUnblockCondition || "result evidence handoff 为 ready 且无 red-line signal", item.owner || "admin_operator"));
    }
    return [ownerLimit("controlled_smoke_release_summary_missing", "补齐脱敏 controlled smoke result evidence 与 release summary", "result evidence handoff 已 ready，release summary 仅包含稳定 alias、计数、风险和脱敏分类")];
}
...
function baseResult(status, input = {}, options = {}, overrides = {}) {
    ...
    const ownerHandoffLimits = overrides.ownerHandoffLimits || [
        ownerLimit(overrides.blockerAlias || "controlled_smoke_release_summary_handoff", status === READY_RELEASE_STATUS ? "交接脱敏 controlled smoke release summary" : "补齐脱敏 controlled smoke release summary", status === READY_RELEASE_STATUS
...
function blockedFromResultEvidence(input = {}, options = {}) {
 ...
 */
function createGatewayProjectionControlledSmokeReleaseSummaryHandoff(input = {}, options = {}) {
    ...
    if (hasSensitiveEvidence(input)) {
        return baseResult("blocked", {}, options, {
            ...
            ownerHandoffLimits: [
                ownerLimit("sanitization_failed", "替换疑似敏感 release summary evidence", "release summary 只保留脱敏 alias、状态、计数和风险分类"),
            ],
        });
    }
    if (!isResultEvidenceReady(input.resultEvidenceHandoffSummary)) {
    ...
    if (unknownAlias) {
        return baseResult("blocked", input, options, {
            ...
            ownerHandoffLimits: [
                ownerLimit("unknown_controlled_smoke_release_summary_alias", "替换未知 release summary alias", "unknown alias 已替换为 spec/test 中定义的稳定 Admin owner release summary alias"),
            ],
        });
    }
    if (NEEDS_USER_ACTION_STATUSES.has(input.releaseSummaryStatus) || aliases.includes("controlled_smoke_release_summary_needs_user_action")) {
        return baseResult("needs-user-action", input, options, {
            ...
            ownerHandoffLimits: [
                ownerLimit("controlled_smoke_release_summary_needs_user_action", "补齐 operator 明确要求的脱敏 release summary 动作或 approval alias", "operator action alias 已清除，release summary 可重新分类", "admin_operator"),
            ],
        });
    }
