...
function baseResult(status, input, options = {}, overrides = {}) {
    ...
    return {
        ...
        ownerHandoffLimits: overrides.ownerHandoffLimits || [
            ownerLimit("controlled_smoke_result_evidence_handoff", "交接脱敏 controlled smoke result evidence", "execution handoff 与 result evidence 均为稳定脱敏摘要且无 red-line signal"),
        ],
...
function createGatewayProjectionControlledSmokeResultEvidenceHandoff(input = {}, options = {}) {
    ...
    if (hasSensitiveEvidence(input)) {
        return baseResult("blocked", {}, options, {
            ...
            ownerHandoffLimits: [
                ownerLimit("sanitization_failed", "替换疑似敏感 result evidence", "结果材料只保留脱敏 alias、状态、计数和风险分类"),
            ],
        });
    }
    ...
    if (unknownAlias) {
        return baseResult("blocked", input, options, {
            ...
            ownerHandoffLimits: [
                ownerLimit("unknown_controlled_smoke_result_alias", "替换未知 result evidence alias", "unknown alias 已替换为稳定 Admin owner handoff alias"),
            ],
        });
    }
