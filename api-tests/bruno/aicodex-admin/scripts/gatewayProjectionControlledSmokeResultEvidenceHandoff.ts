...
function baseResult(status, input, options: LooseRecord = {}, overrides: LooseRecord = {}) {
 ...
 */
function createGatewayProjectionControlledSmokeResultEvidenceHandoff(input: LooseRecord = {}, options: LooseRecord = {}) {
  ...
  const aliases = unique(toArray(input.resultAliases)) as string[];
