...
- `stabilize-admin-go-test-baseline-and-fixtures` 与 `migrate-web-admin-build-toolchain-to-vite` 可以分别进入独立提案评估，但仍不得在同一 change 中混合 Go fixture、Vite、Jest 解耦或依赖升级。
...
2. 可并行实施 `enable-incremental-go-correctness-gates`。
3. 围绕当前 P0 推进 `consolidate-insight-runtime-config-resolution`。
4. 前端 CI change 已收口，可在重新验证最新 Go 测试基线后实施 `stabilize-admin-go-test-baseline-and-fixtures`。
5. 在 fixture/schema registry 边界稳定后，实施 `establish-aicodex-owned-schema-migration-baseline`。
6. 前端 Jest/CI 基线已归档，可单独评估 `migrate-web-admin-build-toolchain-to-vite`；其后再实施 `decouple-web-admin-jest-from-react-scripts`。
7. 重新根据 60 运行态证据判断 lifecycle、API contract、React Router/Testing Library 升级和其它前端架构债是否进入下一阶段。
