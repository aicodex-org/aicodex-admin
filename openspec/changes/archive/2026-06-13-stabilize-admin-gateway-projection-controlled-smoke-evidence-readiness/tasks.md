...
- [x] 2.1 先新增失败的 Node 测试，覆盖 ready、缺少 Admin evidence、缺少 API diagnostics、脱敏缺口、红线信号、full-success 外推和 fallback/owner 指引。
- [x] 2.2 新增 `gatewayProjectionControlledSmokeEvidenceReadiness.js` 纯函数，输出稳定 readiness、reason、owner handoff、最小解除条件、operator next actions 和不能外推边界。
- [x] 2.3 新增 Bruno `Controlled Smoke Evidence Readiness.yml` 只读入口，只消费本地脱敏变量并可按需 fail closed。
...
- [x] 4.2 Archive change，确认 active change 目录进入 `archive/2026-06-13-stabilize-admin-gateway-projection-controlled-smoke-evidence-readiness/`。
