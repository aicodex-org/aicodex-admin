...
- [x] 1.1 编写 proposal/design/tasks 和 delta spec，明确 Admin owner 边界、decision 状态、fail-closed 条件和不能外推边界。
- [x] 1.2 完成实施前 review，并运行 `openspec validate "stabilize-admin-gateway-projection-controlled-smoke-operator-decision-handoff" --strict` 与 `git diff --check`。
...
- [x] 2.2 实现 `gatewayProjectionControlledSmokeOperatorDecisionHandoff.js`，保持本地 dry-run、脱敏输出和 fail-closed 行为。
- [x] 2.3 新增 Bruno 本地入口，运行前完成本地 decision package 生成并在网络请求前中止。
