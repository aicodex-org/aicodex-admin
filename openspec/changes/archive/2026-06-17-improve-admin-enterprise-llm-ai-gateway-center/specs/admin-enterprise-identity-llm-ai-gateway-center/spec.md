...
Admin 企业认证中心 SHALL 在原 `/agents` 路由提供 LLM AI 网关中心工作台，使管理员能够从 LLM AI 区域首屏理解 AI Agent、MCP Server、MCP Store、Entry、Site、Rule、API 网关身份映射和审计记录之间的治理关系。

#### Scenario: 管理员打开 LLM AI 区域
- **WHEN** 已登录管理员访问 `/agents`
- **THEN** 页面展示 LLM AI 网关中心标题、当前 Agent 列表视图摘要、风险待办和配置入口
...
LLM AI 网关中心 SHALL 复用企业认证中心工作台视觉语言，使用安静、信息密度合理的管理台布局，避免营销式 hero、装饰背景和卡片套卡片，并在桌面和窄屏上保持可读可操作。
