...
Admin 身份控制台 Shell SHALL 在组织、用户、应用、Provider、Syncer 等长编辑页中避免外层内容 Card 与页面内部编辑 Card 叠加，页面 SHALL 只保留一个主要编辑页面壳。
...
- **AND** 组织编辑页内部主编辑 Card SHALL 继续承载标题、保存动作和表单内容
...
- **THEN** Shell 根文档 SHALL NOT 因外层内容卡、表单 label gutter 或内部编辑 Card 叠加产生不必要的页面级横向 overflow
