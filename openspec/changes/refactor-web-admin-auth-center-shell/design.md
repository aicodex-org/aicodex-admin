...
**Goals:**
- 将后台默认品牌统一为 AICodeX，并明确品牌展示面的修改范围。
...
- 不在本次变更中对所有 Casdoor 文本做全仓库级语义替换，只覆盖管理员主壳层可见的品牌入口。
...
- 顶部/侧栏主 Logo
- 页脚 `Powered by ...`
- 其他后台首页可直接感知的 Casdoor 品牌文案

实现上优先复用现有 `Setting.getLogo()` 和 `Conf` 配置入口，将默认资源切换为 AICodeX 品牌资源；若正式 logo 资产尚未就绪，则先使用 AICodeX 文本锁定和现有可配置图片回退。若组织级自定义 Logo 已存在，则继续遵循组织级覆盖。这样既满足品牌统一，也不会破坏已有组织定制能力。
...
- [品牌资源未准备好会影响视觉完整性] → 允许先使用配置化文本与临时 logo 占位，再替换为正式 AICodeX 资源。
...
4. 替换 `Conf.js`、`Setting.js`、`index.html`、页脚等壳层品牌默认值为 AICodeX。
...
- AICodeX 默认 Logo 资源文件是否已经确定；若未确定，实现阶段按设计使用文本锁定和可配置图片回退。
