...
#### Scenario: 邀请码新增草稿和编辑操作保持兼容
- **WHEN** 管理员在邀请码列表点击添加
- **THEN** 页面 SHALL 打开带默认值的新增编辑草稿且 SHALL NOT 调用新增、更新或删除邀请码 API
- **WHEN** 管理员在新增页使用保存或保存并返回
- **THEN** 页面 SHALL 仅在该时刻调用新增邀请码 API，并在成功后按原有路由语义进入编辑页或返回列表
- **WHEN** 管理员在新增页使用取消或顶部返回
- **THEN** 页面 SHALL 丢弃草稿并返回列表，且 SHALL NOT 调用新增、更新或删除邀请码 API
- **AND** 复制注册链接、发送邀请、编辑保存的 backend API、请求 payload、路由跳转、默认应用判断和消息提示 SHALL 保持兼容
