# admin-enterprise-ldap-connection-policy Specification

## Purpose
规定 Admin Generic LDAP 与 Active Directory 共用的连接运行时策略，包括 TLS 兼容、固定连接阶段 timeout、失败清理、幂等关闭和 copy-safe 错误/诊断，同时保持既有外部配置与同步契约。
