# admin-enterprise-tls-compatibility Specification

## Purpose

定义 ADFS、Active Directory 同步和 SMTP 的按连接 TLS 兼容策略、存量迁移语义、自定义 CA 边界与管理端配置契约，在维持存量连接行为的同时让新建连接默认使用系统信任，并对无效配置执行 fail-closed。
