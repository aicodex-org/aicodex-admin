...
- **THEN** 命令必须在唯一 schema 中验证 registry 创建、重复同步和表存在性，并在结束时清理该 schema
...
- **THEN** CI 必须以 compatibility 名义运行 legacy DB 基线并单独记录结果，不得将结果等同于 PostgreSQL 主部署验收

#### Scenario: MSSQL 无运行环境
- **WHEN** 当前没有授权 MSSQL 测试环境
- **THEN** 验证记录只能声明编译/方言边界，并必须保留未做运行态覆盖的风险

### Requirement: AICodex-owned schema 集合只有一个注册边界
生产 schema bootstrap、SQLite registry fixture 和 DB integration SHALL 复用同一个窄 AICodex-owned model registry；legacy Casdoor `Sync2` MUST 保持在该 registry 之外。

#### Scenario: 空 SQLite fixture 创建 registry
- **WHEN** 测试对空的内存 SQLite engine 执行 AICodex-owned schema sync
- **THEN** registry 中每个 model 对应的表都必须存在，第二次同步仍成功

#### Scenario: 生产 bootstrap 使用 registry
- **WHEN** Admin 执行当前 `CreateTables()` bootstrap
- **THEN** AICodex-owned 表必须通过同一 registry 注册，legacy Casdoor 表继续使用既有 `Sync2` 路径

#### Scenario: 不建立 versioned migration
- **WHEN** 本 change 完成 schema registry 抽取
- **THEN** 仓库不得新增 schema version、migration history 或替换现有 bootstrap 的 migration executor
