...
显式配置 SHALL 保持现有 mocks、`resetMocks`、fake timers、dynamic imports、CommonJS/ESM、jsdom globals、React 18 legacy warning 和默认单测 timeout 的断言语义，不得通过全局放宽或静默过滤掩盖差异。

#### Scenario: 运行高风险兼容 suites
- **WHEN** 执行 auth、provider、application、Management、runtime env、locale、App lazy import、style topology 与 fake-timer 聚焦 suites
