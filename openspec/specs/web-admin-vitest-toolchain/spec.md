# web-admin-vitest-toolchain Specification

## Purpose
定义 `web-admin` 使用 Vitest 4.1.10 作为唯一单元测试 runner 的可复现契约，覆盖 jsdom/setup、串行执行、mock/module/timer、资产替身、warning 可见性、coverage 与非单测工具链隔离。
