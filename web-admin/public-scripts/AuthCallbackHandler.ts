...
(function() {
  ...
  interface AuthCallbackOAuthParams {
    clientId: string;
    responseType: string;
    redirectUri: string;
    scope: string;
  ...
  interface AuthCallbackLoginBody {
    type: string;
  ...
  interface AuthCallbackApiResponse {
    status: string;
    msg?: string;
    data: string;
    data2?: {
  ...
  function getReactCallbackOrigin(): string {
  ...
  function goToReactFallback(): void {
  ...
  function storeReactFallbackPayload(payload: unknown): void {
    sessionStorage.setItem(reactFallbackPayloadKey, JSON.stringify(payload));
  }

  function getQueryParamsFromState(state: string): string {
  ...
  function getInnerParams(): URLSearchParams | null {
  ...
  function getResponseType(innerParams: URLSearchParams): string {
  ...
  function clearCodeVerifier(state: string): void {
  ...
  function getRawGetParameter(key: string, source: string): string {
  ...
  function getOAuthGetParameters(innerParams: URLSearchParams, queryString: string): AuthCallbackOAuthParams {
    var lowercaseQueries: Record<string, string> = {};
  ...
  function oAuthParamsToQuery(oAuthParams: AuthCallbackOAuthParams | null): string {
    if (!oAuthParams) {
      return "";
    }
  ...
  function createFormAndSubmit(action: string, params: Record<string, string | null | undefined>): void {
  ...
  function extractCallbackCode(params: URLSearchParams): string | null {
    ...
    if (telegramId !== null && (code === null || code === "")) {
      var telegramAuthData: TelegramAuthData = {
  ...
  function shouldFallbackToReact(res: AuthCallbackApiResponse): boolean {
    return res.data === "RequiredMfa" || res.data === "NextMfa" || res.data === "SelectPlan" || res.data === "BuyPlanResult" || Boolean(res.data3);
  }

  function getFromLink(): string {
    return sessionStorage.getItem("from") || "/";
  }

  async function loginCas(body: AuthCallbackLoginBody, casService: string): Promise<AuthCallbackApiResponse> {
  ...
  async function run() {
    ...
    var queryString = getQueryParamsFromState(callbackState);
    ...
    var codeVerifier = getCodeVerifier(callbackState);
    var body = {
      type: responseType,
    ...
    if (codeVerifier) {
      clearCodeVerifier(callbackState);
    }
    ...
    if (responseType === "cas") {
      ...
      if (casService === "") {
        setStatus("Logged in successfully. Now you can visit apps protected by aicodex-admin.", false);
        return;
      }
    ...
    var res = await response.json() as AuthCallbackApiResponse;
    if (res.status !== "ok") {
