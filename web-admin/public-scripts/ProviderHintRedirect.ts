...
(function() {
  ...
  function isProviderVisible(providerItem: ApplicationProviderItem | undefined): boolean {
  ...
  function isProviderVisibleForSignIn(providerItem: ApplicationProviderItem): boolean {
  ...
  } as unknown as Record<string, AuthInfo>;

  function getStateFromQueryParams(applicationName: string, providerName: string, method: string, isShortState: boolean): string {
  ...
  async function getAuthUrl(application: ApplicationRecord, provider: ProviderRecord, method: string, code?: string): Promise<string> {
  ...
  async function getApplication(applicationName: string): Promise<ApiResponse<ApplicationRecord>> {
    return fetchJson<ApiResponse<ApplicationRecord>>("/api/get-application?id=" + encodeURIComponent(applicationName), {
      credentials: "include"
    });
  }

  async function getProviders(applicationName: string, language: string): Promise<ApiResponse<ApplicationProviderItem[]>> {
    return fetchJson<ApiResponse<ApplicationProviderItem[]>>("/api/get-login-providers?application=" + encodeURIComponent(applicationName) + "&acceptLanguage=" + encodeURIComponent(language), {
      credentials: "include"
    });
  }
  ...
  async function run(): Promise<void> {
    ...
    var payload = await response.json() as ApiResponse<ApplicationRecord>;
    ...
    var providerItem = (application.providers || []).find(function(item: ApplicationProviderItem) {
