...
vi.mock("./ManagementPage", () => ({default: () => <main data-testid="management-page" />}));
...
test("renders the admin root shell", async() => {
  ...
  testGlobal.fetch = vi.fn(() => new Promise<Response>(() => undefined)) as typeof fetch;
...
test("recognizes DingTalk organization sync as an admin route for flattened menu selection", () => {
  const appSource = fs.readFileSync(path.join(testFileDirectory, "App.tsx"), "utf8");
