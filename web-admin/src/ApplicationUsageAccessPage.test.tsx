...
describe("ApplicationUsageAccessPage", () => {
  ...
  test("renders the Insight Admin Provider handoff page as the focused usage access content", async() => {
    ...
    expect(view.getByText("在 Admin 部署配置或外部 secret system 维护凭据引用，完成后刷新本页再生成。")).not.toBeNull();
    ...
    expect(view.getByText("诊断详情")).not.toBeNull();
    ...
    expect(await view.findByText("交接能力")).not.toBeNull();
  ...
  test("aligns status, config, and diagnostics by governance item key", async() => {
    ...
    expect(view.queryByLabelText("usage_identity_resolver 凭据引用")).toBeNull();
    fireEvent.click(view.getByText("诊断详情"));
    const gatewayCapabilityBeforeDiagnostic = await view.findByLabelText("gateway_organization_projection capability status");
