...
describe("ApplicationUsageAccessPage", () => {
  ...
  test("renders the service credential governance panel as the focused usage access content", async() => {
    ...
    expect(view.getByText("交接能力")).not.toBeNull();
    ...
    expect(view.getByText("补齐 Admin env/config，重启后刷新本页")).not.toBeNull();
    ...
    expect(view.getByText("需补齐后生成")).not.toBeNull();
    ...
    expect(view.getByLabelText("usage_identity_resolver capability status").textContent).toContain("已就绪");
    expect(view.getByLabelText("gateway_organization_projection capability status").textContent).toContain("缺凭据引用");
    ...
    expect(view.getByText("技术细节")).not.toBeNull();
    ...
    fireEvent.click(view.getByText("技术细节"));
  ...
  test("previews service credential governance handoff after Admin deployment config is ready", async() => {
    ...
    expect(view.getByText("生成 Admin 交接包并交给 Insight 绑定")).not.toBeNull();
    expect(view.getByText("可生成完整包")).not.toBeNull();
    expect(view.getByLabelText("usage_identity_resolver capability status").textContent).toContain("已就绪");
    expect(view.getByLabelText("gateway_organization_projection capability status").textContent).toContain("已就绪");
  ...
  test("aligns status, config, and diagnostics by governance item key", async() => {
    ...
    const view = renderPage();
    await view.findByLabelText("gateway_organization_projection capability status");

    const gatewayCapabilityBeforeDiagnostic = view.getByLabelText("gateway_organization_projection capability status");
  ...
  test("maps diagnostic status labels for all focused service credential governance outcomes", async() => {
    ...
    expect(view.getAllByText("部分缺失").length).toBeGreaterThan(0);
    expect(view.getByText("可生成部分包")).not.toBeNull();
