...
test("includes module-based WeCom organization API paths", () => {
  ...
  expect(paths).toContain("wecom-org-sync/dry-run-preview");
  expect(paths).toContain("wecom-org-sync/dry-run-history");
  ...
  expect(paths).toContain("organization-master-data-quality/remediation-operator-note-persistence-readiness");
  expect(paths).toContain("organization-master-data-quality/remediation-operator-note-readonly-audit-search");
  ...
  expect(paths).toContain("gateway-projection/publish-attempt-retention-cleanup-approval-policy-readiness");
