...
class TelegramLogin extends React.Component<TelegramLoginProps, TelegramLoginState> {
  constructor(props: TelegramLoginProps) {
    super(props);
    this.state = {
      applicationName: "",
      providerName: "",
  ...
  componentDidMount() {
    ...
    ProviderBackend.getProvider("admin", providerName).then((res) => {
      if (res.status === "ok") {
        const provider = res.data as AuthProvider;
          ...
          <Card
            ...
            title={
              <div>
                ...
                />
                {t("login:Sign in with Telegram")}
              </div>
            }
          >
            <div style={{textAlign: "center", padding: "20px"}}>
              <p>{t("login:Click the button below to sign in with Telegram")}</p>
              <div
