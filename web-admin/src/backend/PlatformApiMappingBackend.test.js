...
test("loads platform api mappings with explicit organization query", async() => {
  ...
  await getGatewayProjectionPublishAttemptCleanupApprovalPolicyReadiness("org-a", {
    source: "manual",
    status: "error",
  ...
  await getGatewayProjectionPublishAttemptCleanupApprovalDecisionDraftReadiness("org-a", {
    source: "manual",
    status: "error",
    failureCategory: "gateway_unavailable",
    readinessHash: "dryrun-hash-a",
  ...
  await getGatewayProjectionPublishAttemptCleanupExecutionGateOwnerBoundaryPreflight("org-a", {
    source: "manual",
    status: "error",
    failureCategory: "gateway_unavailable",
    readinessHash: "dryrun-hash-a",
  ...
  expect(global.fetch).toHaveBeenNthCalledWith(
    9,
    "https://admin.example.invalid/api/gateway-projection/publish-attempt-retention-cleanup-approval-policy-readiness?organization=org-a&source=manual&status=error&readinessHash=dryrun-hash-a&dryRunGeneratedAt=2026-06-15T00%3A10%3A00.000Z&maxDryRunAgeSeconds=900&approvalEvidence=dry_run_export_reviewed%2Ccandidate_count_reviewed&limit=100",
    expect.objectContaining({
      method: "GET",
      credentials: "include",
      headers: expect.objectContaining({"Accept-Language": "en"}),
  ...
  expect(global.fetch).toHaveBeenNthCalledWith(
    10,
    "https://admin.example.invalid/api/gateway-projection/publish-attempt-retention-cleanup-approval-decision-draft-readiness?organization=org-a&source=manual&status=error&failureCategory=gateway_unavailable&readinessHash=dryrun-hash-a&dryRunGeneratedAt=2026-06-15T00%3A10%3A00.000Z&maxDryRunAgeSeconds=900&approvalEvidence=dry_run_export_reviewed%2Ccandidate_count_reviewed&limit=100",
    expect.objectContaining({
      method: "GET",
      credentials: "include",
      headers: expect.objectContaining({"Accept-Language": "en"}),
  ...
  expect(global.fetch).toHaveBeenNthCalledWith(
    11,
    "https://admin.example.invalid/api/gateway-projection/publish-attempt-retention-cleanup-execution-gate-owner-boundary-preflight?organization=org-a&source=manual&status=error&failureCategory=gateway_unavailable&readinessHash=dryrun-hash-a&dryRunGeneratedAt=2026-06-15T00%3A10%3A00.000Z&maxDryRunAgeSeconds=900&approvalEvidence=dry_run_export_reviewed%2Ccandidate_count_reviewed&limit=100",
    expect.objectContaining({
      method: "GET",
      credentials: "include",
      headers: expect.objectContaining({"Accept-Language": "en"}),
...
test("loads and records gateway projection cleanup approval audit trail", async() => {
  ...
  expect(global.fetch).toHaveBeenNthCalledWith(
    1,
    "https://admin.example.invalid/api/gateway-projection/publish-attempt-retention-cleanup-approval-policy-readiness?organization=org-a&readinessHash=dryrun-hash-a&limit=20",
    expect.objectContaining({
      method: "GET",
      credentials: "include",
      headers: expect.objectContaining({"Accept-Language": "en"}),
...
test("loads organization directory remediation approval packet operator notes with safe filters", async() => {
  await getOrganizationDirectoryRemediationApprovalPacketOperatorNotes("org-a", {
...
test("loads organization directory remediation operator note readonly audit search with safe filters", async() => {
  await getOrganizationDirectoryRemediationOperatorNoteReadonlyAuditSearch("org-a", {
  ...
  expect(global.fetch).toHaveBeenCalledWith(
    "https://admin.example.invalid/api/organization-master-data-quality/remediation-operator-note-readonly-audit-search?organization=org-a&searchId=operator-note-readonly-audit-search%3Asample&readinessId=operator-note-persistence-readiness%3Asample&readinessHash=sha256%3Areadiness&noteId=operator-note%3Anote&noteHash=sha256%3Anote&packetAuditId=approval-packet-audit%3Apacket&packetHash=sha256%3Apacket&approvalPreviewId=approval-preview%3Apreview&approvalPreviewHash=sha256%3Apreview&draftId=sha256%3Adraft&remediationRunId=run-20260616&actionAlias=mapping_review&entityType=user&keyword=alice&sourceType=wecom&sourceConnectionIdHash=sha256%3Asource&qualityStatus=blocked&reasonCode=mapping_missing&checklistAlias=manual_review_only_required&riskLevel=medium&packetStatus=ready_for_approval&readinessStatus=ready_for_design_review&includeHistorical=true&historyMode=persistent&limit=30&topN=10",
    expect.objectContaining({
      method: "GET",
      credentials: "include",
      headers: expect.objectContaining({"Accept-Language": "en"}),
