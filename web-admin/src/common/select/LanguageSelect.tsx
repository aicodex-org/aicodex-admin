...
function flagIcon(country: string, alt: string): JSX.Element {
  return (
    <img width={24} alt={alt} src={`${Setting.StaticBaseUrl}/flag-icons/${country}.svg`} />
  );
}

class LanguageSelect extends React.Component<LegacyAny, LegacyAny> {
  ...
  getLanguages(): LegacyAny[] {
    return this.props.languages ?? Setting.Countries.map((item: LegacyAny) => item.key);
  }

  getLanguageItems(languages: LegacyAny[]): LegacyAny[] {
    return Setting.Countries.filter((country: LegacyAny) => languages.includes(country.key));
  }
  ...
  triggerLanguageChange(key: string) {
  ...
  renderDropdown(languageItems: LegacyAny[]) {
    const items = languageItems.map((country: LegacyAny) => Setting.getItem(country.label, country.key, flagIcon(country.country, country.alt)));
  ...
  renderTextOnlySelect(languageItems: LegacyAny[]) {
    if (languageItems.length === 0) {
      return null;
    }

    const options = languageItems.map((country: LegacyAny) => Setting.getOption(country.label, country.key));
    let selectedLanguage = this.state.selectedLanguage || Setting.getLanguage();
    if (!options.some((item: LegacyAny) => item.value === selectedLanguage)) {
