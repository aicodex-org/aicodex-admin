...
class RegionSelect extends React.Component<LegacyAny, LegacyAny> {
  ...
  render() {
    ...
    return (
      <Select virtual={false}
        ...
        onChange={(val: LegacyAny) => {this.onChange(val);}}
        filterOption={(input: string, option: LegacyAny) => (option?.label ?? "").toLowerCase().includes(input.toLowerCase())}
        filterSort={(optionA: LegacyAny, optionB: LegacyAny) =>
          (optionA?.label ?? "").toLowerCase().localeCompare((optionB?.label ?? "").toLowerCase())
        }
      >
        {
          Setting.getCountryCodeData().map((item: LegacyAny) => (
