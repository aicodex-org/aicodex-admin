...
class AttributesMapperTable extends React.Component<LegacyAny, LegacyAny> {
  constructor(props: LegacyAny) {
    super(props);

    // transfer the Object to object[]
    const customAttributes = this.props.customAttributes !== null
      ? Object.entries(this.props.customAttributes).map((item: LegacyAny, index: number) => ({
        key: index,
  ...
  updateTable(table: LegacyAny) {
    ...
    const customAttributes: Record<string, LegacyAny> = {};
  ...
  updateField(table: LegacyAny, index: number, attributeName: string, userPropertyName: LegacyAny) {
  ...
  renderTable(table: LegacyAny) {
    const columns = [
      {
        title: t("ldap:LDAP attribute name"),
      ...
      {
        title: t("ldap:User property name"),
